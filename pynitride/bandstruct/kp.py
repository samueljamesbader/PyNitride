...
def kp_6x6_RLB(material, kvecs, strainvec, spin_orbit=True):
    ...
    def for_single_k(k):
        ...
        CB=Eg+np.array([
            ...
            + hbar ** 2 * kz ** 2 / (2 * mezs)\
            + acz * ezz + act * (exx + eyy)])
        return np.concatenate([VB,CB,CB])

    return np.vstack([for_single_k(k) for k in kvecs])

def kp_6x6_CC(material, kvecs, strainvec, spin_orbit=True):
    r""" Solves the 6x6 + 2x2 :math:`k\cdot p` Hamiltonian for a wurtzite crystal.
    ...
    def for_single_k(k):
        ...
        F = Delta1 + Delta2 + Lambda + Theta
        G = Delta1 - Delta2 + Lambda + Theta
        K = A5 * (kx + 1j * ky) ** 2 + D5 * ep
        H = A6 * kz * (kx + 1j * ky) + D6 * ezp

        # CC Eq 41
        hmat = np.matrix([
            [F, -H.conjugate(), -K.conjugate(), 0, 0, 0],
            [-H, Lambda, H.conjugate(), Delta, 0, 0],
            [-K, H, G, 0, Delta, 0],
            [0, Delta, 0, G, -H.conjugate(), -K.conjugate()],
            [0, 0, Delta, -H.conjugate(), Lambda, H.conjugate()],
            [0, 0, 0, -K, H, F]])
