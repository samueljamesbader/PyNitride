...
def lorentz_fit_fwhm(x, y, fitplot=False):
    y = y - np.min(y)
    i = np.argmax(y)
    ...
    # This is unjustified
    mu0 = np.sum(xroi * yroi) / np.sum(yroi)
    gamma0 = np.sqrt(np.sum((xroi - mu0) ** 2 * yroi) / np.sum(yroi))

    def lorentz(x, a, mu, gamma):
        return a / (1+((x-mu)/gamma)**2)

    popt, pcov = curve_fit(lorentz, xroi, yroi, p0=[a0, mu0, gamma0])

    if fitplot:
        plt.figure()
        plt.plot(x, y, label='supplied data')
        plt.plot(xroi, yroi, 'o', label='roi')
        plt.plot(xroi, lorentz(xroi, a0, mu0, gamma0), '--', label='initial guess')
        plt.plot(x, lorentz(x, *popt), linewidth=2, label='fit')
        plt.legend(loc='best')

    return {'a': popt[0], 'mu': popt[1], 'gamma': popt[2],
            'a_unc': np.sqrt(pcov[0,0]),'mu_unc': np.sqrt(pcov[1,1]),'sigma_unc': np.sqrt(pcov[2,2]),
            'vars': ['a','mu','sigma'],
            'popt': popt, 'pcov': pcov, 'func': lambda x: lorentz(x, *popt)}


def fwhm(x, y, fitplot=False):
    y = y - np.min(y)
    i = np.argmax(y)
