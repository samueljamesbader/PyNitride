...
\end{equation}
where $\mu$ and $i$ runs over $\perp, \parallel$. Then $P^\mathrm{ion}_i=\frac{e^*}{V_u}u_i$, giving
\begin{align}
  P^\mathrm{ion}_i =\varepsilon_0\chi_iE_i, \quad \chi_i=\frac{e^{*2}}{\varepsilon_0\mu V_u}\frac{1}{\omega_{0i}^2-\omega^2} 
\end{align}
...
\begin{align}
  \vec u = \frac{V_u}{e^*}\vec P^\mathrm{ion}=\frac{V_u}{e^*}\varepsilon_0\chi\vec E=-\frac{V_u}{e^*}\varepsilon_0\chi\nabla\phi
\end{align}
So
\begin{align}
...
\end{align}
\begin{align}
  \frac{\mu}{V_u}\abs{\vec u}^2 = \frac{\varepsilon_0^2\mu V_u}{e^{*2}}\left( (\chi_\parallel \partial_z\phi)^2 +q^2(\chi_\perp\phi)^2 \right)
\end{align}
\begin{align}
...
\end{align}
where, since $\omega_{LO\perp}$ et al are taken from experiment, we'll average the $(\omega_{LO}^2-\omega_{TO}^2)$ factor over $\perp, \parallel$.  Note: the above equation is equivalent to Stroscio and Dutta Eq 7.78.
...
\begin{equation}
  ...
  \gamma_{\perp     2}=\frac{1}{2k_2}e^{-2k_2t_1}
\end{equation}
\subsubsection{Confined to Region 2}
